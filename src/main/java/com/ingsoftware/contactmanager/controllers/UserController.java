...
public class UserController {
    ...
    @DeleteMapping("/{guid}")
    public ResponseEntity<?> deleteUser(@PathVariable("guid") UUID guid) throws InstanceNotFoundException {
        userService.deleteUser(guid);
        return ResponseEntity.status(HttpStatus.OK).body("Deleted user.");
    }

    @GetMapping("/{guid}")
    public ResponseEntity<?> getUser(@PathVariable("guid") UUID guid) throws InstanceNotFoundException {
        return ResponseEntity.status(HttpStatus.OK).body(userService.getByGuid(guid));
    }

    @PutMapping("/{guid}")
    public ResponseEntity<?> editUser(@RequestBody UserRequestDto userRequestDto, @PathVariable("guid") UUID guid) throws InstanceNotFoundException {
